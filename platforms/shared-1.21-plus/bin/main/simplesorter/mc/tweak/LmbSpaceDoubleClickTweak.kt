...
class LmbSpaceDoubleClickTweak : MouseTweakModule {
    ...
    override fun onMouseClicked(screen: HandledScreen<*>, slot: Slot?, button: Int, isShift: Boolean): Boolean {
        ...
        if (isDoubleClick) {
            ...
            // Find all slots in this specific inventory (e.g. all slots in the chest)
            for (s in handler.slots) {
                // If the slot belongs to the inventory we double-clicked, and it has an item, move it!
            ...
            return true // Cancel vanilla click behavior
        }
        
        return true // Cancel single click too if space is held, or let it fall through?
        // Actually, if we cancel single click, they can't unintentionally pick up an item
        // while preparing the double click. This is safer.
    }
}
